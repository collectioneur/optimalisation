...
matrix ff5R(matrix x, matrix ud1, matrix ud2)
{
    ...
    if (l < l_min)
        penalty += pow(l_min - l, 2);
    if (l > l_max)
        penalty += pow(l - l_max, 2);
    if (d < d_min)
        penalty += pow(d_min - d, 2);
    if (d > d_max)
        penalty += pow(d - d_max, 2);
...
matrix ff6T(matrix x, matrix ud1, matrix ud2)
{
    ...
    double x1 = x(0);
    double x2 = x(1);
    y(0) = pow(x1, 2) + pow(x2, 2) - cos(2.5 * M_PI * x1) - cos(2.5 * M_PI * x2) + 2.0;
    return y;
}
...
matrix df6(double t, matrix Y, matrix ud1, matrix ud2)
{
    ...
    // m2*a2 - b2*(v1-v2) - k2*(x1-x2) = F
    // => a2 = (F + b2*(v1-v2) + k2*(x1-x2)) / m2
    dY(3) = (F + b2 * (v1 - v2) + k2 * (x1 - x2)) / m2;
